...
export function AdminControlsSection({
  ...
  return (
    <section className="rounded-lg border border-slate-800 bg-slate-950/75 p-4 shadow-lg shadow-black/20 sm:p-5">
      ...
      {writesDisabled ? (
        <div className="mt-3 rounded-lg border border-amber-400/20 bg-amber-400/10 px-4 py-3 text-sm text-amber-100/85">
      ...
      <div className="mt-4 grid gap-3 lg:grid-cols-2">
        {controlKeys.map((key) => {
          ...
          return (
            <div key={key} className="rounded-lg border border-slate-800 bg-slate-900/55 p-4">
              ...
              <label className="mt-4 block">
                ...
                <textarea
                  ...
                  disabled={isBusy}
                  rows={3}
                  className="mt-2 w-full resize-none rounded-md border border-slate-700 bg-slate-950/70 px-3 py-2 text-sm text-slate-200 outline-none transition-colors placeholder:text-slate-600 focus:border-emerald-400/50 disabled:cursor-not-allowed disabled:opacity-50"
              ...
              <div className="mt-4 flex flex-col gap-2 sm:flex-row">
                ...
                <button
                  ...
                  onClick={() => onUpdate(key, enabled)}
                  disabled={isBusy}
                  className="inline-flex h-9 flex-1 items-center justify-center gap-2 rounded-md border border-emerald-400/25 bg-emerald-400/10 px-3 text-xs font-semibold uppercase tracking-widest text-emerald-200 transition-colors hover:bg-emerald-300 hover:text-slate-950 disabled:cursor-not-allowed disabled:opacity-50"
                >
