...
export async function bootstrap(req: Request, res: Response, next: NextFunction) {
  try {
    ...
    const contracts = {
      ...
      vault: env.DEPOSIT_VAULT_ADDRESS || env.LOCK_VAULT_ADDRESS || (productionMainnetConfigRequired ? '' : env.VAULT_ADDRESS) || '',
      deposit_vault: env.DEPOSIT_VAULT_ADDRESS || env.LOCK_VAULT_ADDRESS || '',
      deposit_contract_kind: env.DEPOSIT_VAULT_ADDRESS ? 'deposit_vault' : env.LOCK_VAULT_ADDRESS ? 'lock_vault' : '',
      deposit_vault_jetton_wallet: env.DEPOSIT_VAULT_JETTON_WALLET_ADDRESS || env.LOCK_VAULT_JETTON_WALLET_ADDRESS || '',
      deposit_season_id: env.DEPOSIT_VAULT_SEASON_ID || '1',
